...
let activeConfig = Object.keys(SITE_CONFIGS).find(domain => currentHostname.includes(domain)) 
  ? SITE_CONFIGS[Object.keys(SITE_CONFIGS).find(domain => currentHostname.includes(domain))!] 
  : null;
  ...
  try {
    chrome.storage.local.get(['settings'], (result) => {
      try {
        ...
        const adapterEnabled = settings?.siteAdapters?.[Object.keys(SITE_CONFIGS).find(domain => currentHostname.includes(domain))!] ?? true;
  ...
  const interval = setInterval(() => {
    try {
      ...
      if (input && !existing) {
        ...
        detectionAttempts++;
        // If we cannot find the input box after 5 attempts (10 seconds), render a fixed fallback button in the window corner
...
function findInputContainer(inputEl: HTMLElement): HTMLElement {
  ...
  let best = inputEl.parentElement;
...
function injectTriggerButton(isFallback: boolean) {
  ...
  if (isFallback) {
    // Large circular floating trigger in window corner (Cal.com Black CTA)
    ...
    // Premium pill badge sitting above the input container (Cal.com badge-pill)
    Object.assign(trigger.style, {
      zIndex: '9999',
      cursor: 'pointer',
      display: 'inline-flex',
...
function renderSkillsList(skills: any[], container: HTMLDivElement, search: string, isFallback: boolean) {
  container.innerHTML = '';
  ...
  filtered.forEach(skill => {
    ...
    item.innerHTML = `
      <div style="font-size:11px;font-weight:600;margin-bottom:2px;font-family:Inter, system-ui, sans-serif;display:flex;justify-content:space-between;color:#111111;">
        ...
        <span style="font-size:8px;color:#111111;font-weight:bold;opacity:0.8">${isFallback ? 'COPY' : 'INJECT'}</span>
      </div>
      <div style="font-size:9.5px;color:#6b7280;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:Inter, system-ui, sans-serif;">${skill.description}</div>
    `;
...
function injectSkillContent(skill: any) {
  const inputEl = document.querySelector(activeConfig!.inputSelector) as HTMLElement;
  if (!inputEl) {
    copyToClipboardFallback(skill);
    return;
  ...
  const formattedPrompt = `[AGENT SKILL DEFINITION]
Name: ${skill.name}
...
`;
  ...
  try {
    // ProseMirror standard command execution
    ...
    try {
      ...
      console.warn('DOM Injection failed. Falling back to clipboard.', e);
      copyToClipboardFallback(skill);
    }
  }
}

function copyToClipboardFallback(skill: any) {
  const formattedPrompt = `[AGENT SKILL DEFINITION]
Name: ${skill.name}
Description: ${skill.description}
Instructions:
